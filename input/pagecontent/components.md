...
`SubscriptionTopic` resources contain information that is difficult to model without an appropriate resource to start from.  In R4, servers SHOULD use the standard cross-version profile [`profile-SubscriptionTopic`](http://hl7.org/fhir/5.0/StructureDefinition/profile-SubscriptionTopic) (from the `hl7.fhir.uv.xver-r5.r4` package) on the [Basic](http://hl7.org/fhir/R4/basic.html) resource to provide a standardized representation of `SubscriptionTopic` content.  This cross-version profile uses extensions to carry the topic metadata, resource triggers, filter definitions, and notification shape, mirroring the structure of the R5 `SubscriptionTopic` resource.  Earlier versions of this guide defined a local profile for the same purpose, but that has been replaced by the standard cross-version definition.  For a complete example, see [Backported SubscriptionTopic: R4 Encounter Complete](Basic-r4-encounter-complete.html).
